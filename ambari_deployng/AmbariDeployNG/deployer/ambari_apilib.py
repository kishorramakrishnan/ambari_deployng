...
class APICoreLib:
    ...
    def http_put_post_request(self,uri, data, requestType, **kwargs):
        ...
        if requestType=='PUT':
            if contType is not None:
                response = requests.put(url=url, data=data, auth=basic_auth, headers={'X-Requested-By': 'ambari', 'content-type': contType}, verify=False)
            else:
                response = requests.put(url=url, data=data, auth=basic_auth, headers={'X-Requested-By': 'ambari'}, verify=False)

            return response
        elif requestType=='POST':
            if contType is not None:
                response = requests.post(url=url, data=data, auth=basic_auth, headers={'X-Requested-By': 'ambari', 'content-type': contType}, verify=False)
            else:
                response = requests.post(url=url, data=data, auth=basic_auth, headers={'X-Requested-By': 'ambari'}, verify=False)
            return response
        else:
    ...
    def get_services_to_host_to_components_data(self, service):
        uri = "/api/v1/clusters/"+str(self.AmbariProperties.get("CLUSTER_NAME"))+"/services?fields=components/host_components"
        result = self.http_get_request(uri)
        data = json.loads(result._content)
        serviceToHostDict = {}
        for i in range(0,len(data['items'])):
            ...
            for j in range(0, len(data['items'][i]['components'])):
                for k in range(0, len(data['items'][i]['components'][j]['host_components'])):
                    if not '_CLIENT' in str(data['items'][i]['components'][j]['host_components'][k]['HostRoles']['component_name']):
                        hostname = str(data['items'][i]['components'][j]['host_components'][k]['HostRoles']['host_name'])
                        if hostname in hosts :
                            components = hosts[hostname]
                            components.append(str(data['items'][i]['components'][j]['host_components'][k]['HostRoles']['component_name']))
                        else:
                            components = []
                            components.append(str(data['items'][i]['components'][j]['host_components'][k]['HostRoles']['component_name']))
        ...
        for i in range(0,len(data['items'])):
            ...
            for j in range(0, len(data['items'][i]['components'])):
                for k in range(0, len(data['items'][i]['components'][j]['host_components'])):
                    if not '_CLIENT' in str(data['items'][i]['components'][j]['host_components'][k]['HostRoles']['component_name']):
                        hostname = str(data['items'][i]['components'][j]['host_components'][k]['HostRoles']['host_name'])
                        componentname = str(data['items'][i]['components'][j]['host_components'][k]['HostRoles']['component_name'])
    ...
    def get_services_to_components_data(self, service):
        ...
        for i in range(0,len(data['items'])):
            ...
            for j in range(0, len(data['items'][i]['components'])):
                for k in range(0, len(data['items'][i]['components'][j]['host_components'])):
                    if not '_CLIENT' in str(data['items'][i]['components'][j]['host_components'][k]['HostRoles']['component_name']):
                        component = str(data['items'][i]['components'][j]['host_components'][k]['HostRoles']['component_name'])
    ...
    def start_stop_component(self, component, host, state):
        ...
        url = '/api/v1/clusters/'+str(self.AmbariProperties.get('CLUSTER_NAME'))+'/hosts/'+host+'/host_components/'+component
    ...
    def get_services_components_with_stale_configs(self):
        url = '/api/v1/clusters/'+str(self.AmbariProperties.get('CLUSTER_NAME'))+'/host_components?HostRoles/stale_configs=true&fields=HostRoles/service_name,HostRoles/state,HostRoles/host_name,HostRoles/stale_configs,&minimal_response=true'
        response = self.http_get_request(url)
        json_data = json.loads(response._content)
    ...
    def get_service_current_state(self, service):
        url = '/api/v1/clusters/' + str(self.AmbariProperties.get('CLUSTER_NAME')) + '/services/' + service + '?fields=ServiceInfo/state'
        response = self.http_get_request(url)
        json_data = json.loads(response._content)
    ...
    def get_service_config(self, serviceName, config_type, config):
        ...
        url = '/api/v1/clusters/'+str(self.AmbariProperties.get('CLUSTER_NAME'))+'/configurations?type='+config_type+'&tag='+tag
        response = self.http_get_request(url)
        json_data = json.loads(response._content)
    ...
    def validate_service_configs(self, serviceName, config_type, configs_dict):
        self.logger.info("Validating the config changes for the service "+ serviceName+ " and config type "+config_type+" : ")
        url = '/api/v1/clusters/'+str(self.AmbariProperties.get('CLUSTER_NAME'))+'?fields=Clusters/desired_configs'
        response = self.http_get_request(url)
        json_data = json.loads(response._content)
        tag = json_data['Clusters']['desired_configs'][config_type]['tag']
        ...
        self.logger.info("Validation passed for the config changes for the service "+ serviceName+ " and config type "+config_type+" : ")
    ...
    def modify_service_configs(self, serviceName, config_type, configs_dict):
        ...
        try:
            ...
            json_data = '{"Clusters":{"desired_config":{"tag":"'+new_tag+'", "type":"'+config_type+'", "properties":'+str(json.dumps(json_data['items'][0]['properties']))+'}}}'
        except KeyError:
            properties={}
            for config in configs_dict.keys():
                properties[config] = configs_dict.get(config)
            json_data = '{"Clusters":{"desired_config":{"tag":"'+new_tag+'", "type":"'+config_type+'", "properties":'+str(json.dumps(properties))+'}}}'
    ...
    def modify_config(self, config_type, nodeSelection):
        ...
        # Step 2: Read the config type with correct tag
        url = '/api/v1/clusters/'+str(self.AmbariProperties.get('CLUSTER_NAME'))+'/configurations?type='+filename+'&tag='+tag
        response = self.http_get_request(url)
        json_data = json.loads(response._content)
        try:
            ...
            json_data = '{"Clusters":{"desired_config":{"tag":"'+new_tag+'", "type":"'+filename+'", "properties":'+str(json.dumps(json_data['items'][0]['properties']))+', "properties_attributes":'+str(json.dumps(json_data['items'][0]['properties_attributes']))+'}}}'
        except KeyError:
            properties={}
            for fname in config_type.keys():
                for config in config_type.get(fname).keys():
                    properties[config] = config_type.get(fname).get(config)
            json_data = '{"Clusters":{"desired_config":{"tag":"'+new_tag+'", "type":"'+fname+'", "properties":'+str(json.dumps(properties))+'}}}'
        # Step 3: Save a new version of the config and apply it using one call
        url = '/api/v1/clusters/'+str(self.AmbariProperties.get('CLUSTER_NAME'))
        response = self.http_put_post_request(url, json_data, 'PUT')
    ...
    def restoreOrigConfig(self, serviceName, version):
        ...
        service_config_version_data['cluster']['desired_service_config_versions']['service_config_version_note'] = 'Restoring Original Configuration'
    ...
    def get_current_stack_version(self):
        uri = "/api/v1/clusters/"+str(self.AmbariProperties.get("CLUSTER_NAME"))+"/stack_versions?ClusterStackVersions/state=CURRENT&fields=*"
        result = self.http_get_request(uri)
        data = json.loads(result._content)
        for i in range(0,len(data['items'])):
            for j in range(0, len(data['items'][i]['repository_versions'])):
                ...
                uri = "/api/v1/clusters/"+str(self.AmbariProperties.get("CLUSTER_NAME"))+"/stack_versions" + uri.split('stack_versions')[1]
                print uri
